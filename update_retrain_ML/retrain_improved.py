...
def main():
    ...
    selector_kbest = SelectKBest(score_func=mutual_info_classif, k=min(33, X_train_t1.shape[1]))
    ...
    # STEP 6: OPTIMIZE CLASSIFICATION THRESHOLD FOR LIGHTGBM
    print("\n[STEP 6] OPTIMIZING CLASSIFICATION THRESHOLD FOR LIGHTGBM...")
    from sklearn.metrics import precision_recall_curve
    
    lgbm_model = models["LightGBM_Regulated"]
    
    # 1. Predict probabilities on validation set
    y_val_probs = lgbm_model.predict_proba(X_val_final)[:, 1]
    
    # 2. Sweep all feasible thresholds
    ...
    print(f"  Validation F1-score with default threshold (0.5000): {f1_score(y_val, (y_val_probs >= 0.5).astype(int)):.4f}")
    print(f"  Optimal Threshold found: {best_threshold:.4f} (Validation F1: {best_f1:.4f})")

    # STEP 7: EVALUATE ON TEST SET
    print("\n[STEP 7] EVALUATING ON TEST SET (20%)...")
    y_train_probs = lgbm_model.predict_proba(X_train_final)[:, 1]
    y_test_probs = lgbm_model.predict_proba(X_test_final)[:, 1]
    
    # Predict using default threshold (0.5)
    y_test_pred_default = (y_test_probs >= 0.5).astype(int)
    acc_train_default = accuracy_score(y_train, (y_train_probs >= 0.5).astype(int))
    acc_test_default = accuracy_score(y_test, y_test_pred_default)
    
    # Predict using optimal threshold
    y_test_pred_opt = (y_test_probs >= best_threshold).astype(int)
    acc_train_opt = accuracy_score(y_train, (y_train_probs >= best_threshold).astype(int))
    acc_test_opt = accuracy_score(y_test, y_test_pred_opt)
    
    print("--- Default Threshold (0.5000) ---")
    print(f"Accuracy Train Set: {acc_train_default*100:.2f}%")
    print(f"Accuracy Test Set : {acc_test_default*100:.2f}%")
    print(classification_report(y_test, y_test_pred_default, target_names=["Human (0)", "AI (1)"]))
    
    print(f"--- Optimal Threshold ({best_threshold:.4f}) ---")
    print(f"Accuracy Train Set: {acc_train_opt*100:.2f}%")
    print(f"Accuracy Test Set : {acc_test_opt*100:.2f}%")
    print(classification_report(y_test, y_test_pred_opt, target_names=["Human (0)", "AI (1)"]))

    # STEP 8: SERIALIZE ASSETS TO saved_models_44_features_improved
    print("\n[STEP 8] SERIALIZING ASSETS TO saved_models_44_features_improved...")
    
    # Save Feature Order inside Scaler for downstream alignment
    scaler.feature_names_in_ = np.array(feature_order, dtype=object)
    
    # 1. Models
    joblib.dump(lgbm_model, os.path.join(SAVE_DIR, "LightGBM_Regulated.pkl"))
    ...
    print("Saved all models, scaler, features, and threshold.")
