...
def strip_metadata_headers(code_text):
    ...
    # Loại bỏ cả các license block cơ bản nếu cần (tùy chọn)
...
class CppFeatureExtractorV8:
    ...
    """
    def __init__(self):
        # 20 features được chọn trong LightGBM_Regulated
        self.feature_names = [
    ...
    def extract(self, code_text):
        """Trả về 1 dictionary (hoặc list) chứa 32 features."""
        ...
        # brace style: đếm ngoặc nhọn ở cuối dòng (K&R) vs đầu dòng (Allman)
        ...
        # 2. Naming (6-10)
        # Giả lập tìm biến/hàm (đơn giản qua regex \b[a-zA-Z_]\w*\b)
        identifiers = re.findall(r'\b[a-zA-Z_]\w*\b', code_text)
        keywords = {'int', 'float', 'double', 'char', 'void', 'if', 'else', 'for', 'while', 'return', 'class', 'struct', 'public', 'private', 'protected', 'virtual', 'template', 'namespace', 'using', 'std', 'cout', 'cin', 'endl', 'include'}
        ...
        features['keyword_to_identifier_ratio'] = len([w for w in identifiers if w in keywords]) / len(identifiers) if identifiers else 0

        # 3. Complexity & Halstead via lizard (11-18)
        # Lizard không cho Halstead trực tiếp, ta sẽ giả lập xấp xỉ Halstead hoặc tự đếm
        lizard_res = lizard.analyze_file.analyze_source_code("temp.cpp", code_text)
        
        if lizard_res.function_list:
            features['avg_cyclomatic_complexity'] = sum(f.cyclomatic_complexity for f in lizard_res.function_list) / len(lizard_res.function_list)
            features['num_functions'] = len(lizard_res.function_list)
            features['avg_function_loc'] = sum(f.length for f in lizard_res.function_list) / len(lizard_res.function_list)
        else:
            features['avg_cyclomatic_complexity'] = 1.0
            features['num_functions'] = 0.0
            features['avg_function_loc'] = 0.0
            
        # Halstead Approximation (đếm operators và operands đơn giản)
        ...
        # Maintainability Index (MI) xấp xỉ
        mi = 171 - 5.2 * math.log(volume if volume > 0 else 1) - 0.23 * features['avg_cyclomatic_complexity'] - 16.2 * math.log(num_lines if num_lines > 0 else 1)
        features['maintainability_index'] = max(0, mi * 100 / 171) # scale 0-100
        ...
        features['code_to_comment_ratio'] = (num_lines - comment_loc) / comment_loc if comment_loc > 0 else num_lines
        
        # Max nesting depth xấp xỉ qua thụt lề
        indents = [len(line) - len(line.lstrip()) for line in lines if line.strip()]
        # Giả định 1 indent = 4 spaces hoặc 1 tab
        ...
        features['modern_cpp_ratio'] = len(re.findall(r'auto\s+|constexpr\s+|nullptr', code_text)) / len(identifiers) if identifiers else 0
        features['const_usage_ratio'] = len(re.findall(r'\bconst\b', code_text)) / len(vars_only) if vars_only else 0
        ...
        # Trả về dict theo đúng thứ tự 32 features
