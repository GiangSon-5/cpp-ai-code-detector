...
try:
    # Tìm PID đang chiếm cổng 8081
    result = subprocess.check_output(["lsof", "-t", "-i:8081"], stderr=subprocess.STDOUT)
    for pid in result.decode().split():
        print(f"⚠️ [SYSTEM] Đang tắt process cũ (PID: {pid}) đang chiếm cổng 8081...")
...
if __name__ == "__main__":
    print("🧠 Khởi động Local GPU Agent tại port 8081...")
    uvicorn.run("src.local_gpu_agent.main:app", host="127.0.0.1", port=8081, log_level="info")
