"""
engine.py — Core AI Engine: ModelManager, RoBERTa Ensemble, LIG Attribution
Wrap từ extract_1.py §3-§5 (lines 165-470)

Colab chỉ trả về raw tokens và attribution scores.
HTML rendering và ExpertExplainer logic đã chuyển sang Local Brain.
"""
...
class ModelManager:
    ...
    def load_resources(self):
        ...
        colab_log("info", "engine", "load_resources", "Starting model load")
        ...
        try:
            ...
            colab_log("warning", "engine", "load_resources",
        ...
        msg = f"Ready. Loaded {len(self.oop_models)} OOP & {len(self.normal_models)} Normal models."
        print(f"✅ [SYSTEM] {msg}")
        colab_log("info", "engine", "load_resources", msg,
                  latency_ms=round(latency, 1),
                  oop_count=len(self.oop_models),
                  normal_count=len(self.normal_models))

    def _load_from_disk(self, root_path, label):
        """Load K-fold models from a directory."""
        loaded = []
        try:
            if not os.path.exists(root_path):
                colab_log("warning", "engine", "_load_from_disk",
            ...
            for p in tqdm(folds, desc=f"Loading {label}"):
                ...
                model.eval()
                loaded.append(model)
            colab_log("info", "engine", "_load_from_disk",
                      f"Loaded {len(loaded)} {label} models")
        except Exception as e:
            print(f"❌ Error loading {label}: {e}")
            colab_log("error", "engine", "_load_from_disk",
    ...
    def cleanup(self):
        """Giải phóng VRAM"""
        colab_log("info", "engine", "cleanup", "Releasing VRAM")
        del self.oop_models
...
def run_roberta_engine(code_text, model_type_name, manager):
    ...
    """
    t0 = time.perf_counter()
    colab_log("info", "engine", "run_roberta_engine",
              f"Starting inference: {model_type_name}",
    ...
    colab_log("info", "engine", "run_roberta_engine",
              f"Tokenized: {total_tokens} tokens, {len(chunks_info)} chunks")
    ...
    if not models_in_ram:
        colab_log("error", "engine", "run_roberta_engine",
                  f"No preloaded models for {model_type_name}")
    ...
    for fold_idx, model_cpu in enumerate(tqdm(models_in_ram, desc=f"Running {model_type_name}")):
        ...
        try:
            ...
            for batch_start in range(0, len(chunks_info), ENGINE_BATCH_SIZE):
                ...
                if fold_idx == 0:
                    ...
                    attrs = lig.attribute(
                        ...
                        target=1,
                        n_steps=5, # Reduced for massive speedup
                    ...
                    attrs_np = np.zeros((len(batch_chunks), max_len_in_batch))
            ...
            fold_latency = (time.perf_counter() - fold_t0) * 1000
            colab_log("info", "engine", "run_roberta_engine",
                      f"Fold {fold_idx+1} complete",
                      fold_idx=fold_idx+1, latency_ms=round(fold_latency, 1))

        except Exception as e:
            print(f"⚠️ Error in Fold {fold_idx + 1}: {e}")
            colab_log("error", "engine", "run_roberta_engine",
                      f"Fold {fold_idx+1} failed: {e}")
            try:
                model.cpu()
                torch.cuda.empty_cache()
                gc.collect()
