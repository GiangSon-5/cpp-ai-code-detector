...
async def analyzer_node(state: AgentState):
    ...
    print(f"⚙️ [LOCAL AGENT] Analyzer Node: Calling Colab APIs...")

    # 1. Gọi RoBERTa (Lấy Raw Data thay vì HTML)
    print(f"   ▶️  Calling RoBERTa API (Raw Data)...")
    roberta_res = await client.get_roberta_data(state["code_input"], decision)
    bert_score = roberta_res.get("final_score", 0.5)
    
    # 2. Gọi LightGBM (Qua API lên Colab)
    print(f"   ▶️  Calling LightGBM API...")
    lgbm_score = await client.get_lightgbm_score(state["code_input"])
    if lgbm_score is None: lgbm_score = bert_score
...
async def critique_node(state: AgentState):
    print("📝 [LOCAL AGENT] Critique Node: Calling vLLM Proxy...")
    result = state["final_output"]
